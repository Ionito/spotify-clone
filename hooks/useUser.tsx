    ...
    if (user && !isLoadingData && !userDetails && !subscription) {
      ...
      Promise.allSettled([getUserDetails(), getSuscription()]).then(
        (results) => {
          ...
          const userSubscription = results[1];

          if (userDetails.status === "fulfilled") {
            setUserDetails(userDetails.value.data as UserDetails);
          }

          if (userSubscription.status === "fulfilled") {
            setSubscription(userSubscription.value.data as Subscription);
          }
...
export const useUser = () => {
  ...
  if (context === undefined) {
    throw new Error("use inside userContext Provider");
  }
  return context;
};
